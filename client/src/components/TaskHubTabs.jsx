...
const tabClass = (active) =>
  `w-[5.5rem] text-center py-1.5 rounded-full text-xs font-semibold uppercase tracking-wide transition-colors ${
...
export default function TaskHubTabs({ userId }) {
  ...
  return (
    <div className="inline-flex rounded-full bg-surface-container-low p-1 border border-outline-variant/20 shrink-0">
      <Link to={`/tasks/${userId}`} className={tabClass(!isProjects)}>
        Tasks
      </Link>
